...
from ..migoto_io.blender_interface.objects import link_object_to_collection
from ..migoto_io.data_model.data_model import DataModel
from ..migoto_io.data_model.byte_buffer import NumpyBuffer, MigotoFmt, AbstractSemantic, Semantic, enum_equal
...
class ObjectImporter:
    @staticmethod
    def _component_sort_key(filename):
        match = re.findall(r'component[ -_]*([0-9]+)', filename.lower())
        if len(match) == 1:
            return int(match[0])
        return 999999
    ...
    def import_object(self, operator, context, cfg):
        ...
        imported_components = []
        for filename in sorted(os.listdir(object_source_folder), key=self._component_sort_key):
            ...
            if not ib_path.is_file():
                raise ConfigError('object_source_folder', f'Specified folder is missing .ib file for {fmt_path.stem}!')
            if not vb_path.is_file():
                raise ConfigError('object_source_folder', f'Specified folder is missing .vb file for {fmt_path.stem}!')
        ...
        imported_objects = [obj for obj, _component_name in imported_components]
        setup_materials(object_source_folder, imported_objects)

        col = new_collection(object_source_folder.stem)
        for obj, _component_name in imported_components:
        ...
        with open(fmt_path, 'r') as fmt, open(ib_path, 'rb') as ib, open(vb_path, 'rb') as vb:
            ...
            if color1 is not None and color1.format == DXGIFormat.R16G16_UNORM:
                for sem in migoto_fmt.vb_layout.semantics:
                    if enum_equal(sem.abstract.enum, Semantic.Color) and sem.abstract.index == 1:
                        sem.abstract = AbstractSemantic(Semantic.TexCoord, 1)
                        sem.format = DXGIFormat.R16G16_FLOAT
                    elif enum_equal(sem.abstract.enum, Semantic.TexCoord) and sem.abstract.index >= 1:
